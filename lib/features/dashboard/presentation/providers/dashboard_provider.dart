...
class DashboardController extends Notifier<DashboardState> {
  ...
  Future<bool> acceptJob(String orderId) async {
    ...
    if (success) {
      state = state.copyWith(acceptedJobIds: {...state.acceptedJobIds, orderId});
      await fetchUpcomingJobs();
    } else {
      state = state.copyWith(isLoading: false, error: "Failed to accept job");
    }
    return success;
  }
  ...
  Future<bool> rejectJob(String orderId, String reason) async {
    state = state.copyWith(isLoading: true);
    final success = await ApiService.agentApprovalOrder(orderId, "REJECTED", reason: reason);
    if (success) {
      await fetchUpcomingJobs();
    } else {
