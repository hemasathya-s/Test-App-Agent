...
class ApiService{
 ...
 //Agent Approval for Order Assignment
  ...
  static Future<ApiResponse<String>> sendOtp(String mobileNumber) async {
    try {
  ...
  static Future<ApiResponse<VerifyOtpResponse>> verifyOtp({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('🔍 [VerifyOtp] Could not resolve tokens from 200 response.');
        print('🔍 [VerifyOtp] Full JSON dump: $json');
...
//   }
  ...
  static Future<AgentApiResult<AgentRegistrationResponse>> registerAgent({
    required AgentRegistrationRequest request,
    File? profileImage,
    File? aadharDoc,
    File? panCard,
  ...
  static Future<AgentApiResult<bool>> updateAgentProfile(
      String userId,
 ...
 static AgentApiResult<bool> _handleUpdateResponse(http.Response response) {
 ...
 static Future<AgentApiResult<bool>> logoutUser() async {
    try {
      final userId = await getUserId();
      final accessToken = await _getAccessToken();
